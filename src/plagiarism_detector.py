...
class PlagiarismDetector:
        ...
        for file_path in folder.iterdir():
            if file_path.suffix.lower() in supported_extensions:
                try:
                    ...
                    print(
                        f"✓ Загружен: {file_path.name} ({len(content)} символов)")
    ...
    def visualize_results(self, similarity_matrices: Dict[str, np.ndarray]) -> None:
        ...
        try:
            ...
            titles = ['Cosine Similarity', 'LCS Similarity',
                      'N-gram Similarity', 'Combined Similarity']
    ...
    def generate_report(self, similarity_matrices: Dict[str, np.ndarray]) -> None:
        ...
        if potential_plagiarism:
            ...
            for doc1, doc2, combined, cosine, lcs, ngram in sorted(potential_plagiarism, key=lambda x: x[2], reverse=True):
                print(
        ...
        if n > 1:
            ...
            if n > 1 and max_similarity > 0:
                ...
                for i, j in zip(indices[0], indices[1]):
                    if i < j:
                        print(
                            f"  {filenames[i]} ↔ {filenames[j]}: {max_similarity:.2%}")
...
def create_test_documents(folder_name: str = "test_documents"):
    ...
    print(
        f"✓ Создано {len(created_files)} тестовых документов в папке '{folder_name}'")
    return folder_name
...
def create_test_documents_english(folder_name: str = "english_documents"):
    ...
    print(
        f"✓ Created {len(created_files)} test documents in '{folder_name}' folder")
    return folder_name
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        detector = PlagiarismDetector(
            min_similarity_threshold=0.4, language='russian')
    else:
        print("\n🇬🇧 Using English language")
        test_folder = create_test_documents_english()
        detector = PlagiarismDetector(
            min_similarity_threshold=0.4, language='english')
