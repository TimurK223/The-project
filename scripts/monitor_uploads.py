...
class UploadHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if file_path.stat().st_size > 100 * 1024 * 1024:  # 100MB
            logger.warning(f"Файл {file_path.name} слишком большой (>{file_path.stat().st_size/1024/1024:.1f}MB)")
            return
    ...
    def save_results(self, filename: str, results: Dict):
        ...
        try:
            ...
            with open(result_path, 'w', encoding='utf-8') as f:
                json.dump(results_with_metadata, f, 
                         ensure_ascii=False, 
                         indent=2,
                         default=str)
        ...
        try:
            ...
            if csv_path.exists():
                ...
                df = pd.DataFrame(columns=[
                    'timestamp', 'filename', 'total_documents', 
                    'potential_cases', 'max_similarity', 'avg_similarity'
                ])
            ...
            new_row = {
                ...
                'potential_cases': results.get('summary', {}).get('potential_plagiarism_cases', 0),
